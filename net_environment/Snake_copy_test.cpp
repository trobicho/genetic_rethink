...
/*   Updated: 2019/10/31 15:01:33 by trobicho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Snake::sensor_update()
{
	...
	if (m_extra_sensor)
	{
		m_sensor[m_nbOutPerDir * m_nb_direction] = m_snake.len / 300.0;
		if (m_extra_sensor > 1)
		{
			if (m_bFood)
				m_sensor[m_nbOutPerDir * m_nb_direction + 1] = 
					(m_starving + m_snake.len - m_moveNoEat) / (double)m_starving;
			else
				m_sensor[m_nbOutPerDir * m_nb_direction + 1] = 0.0;
		}
	}
	if (m_bias)
		m_sensor.back() = 1.0;
